...
export interface BusPass {
  ...
  passType: 'Monthly' | 'Quarterly' | 'Yearly' | 'Temporary';
...
export const SHIFT_ROUTES: Record<string, Record<string, string[]>> = {
  "1st Shift": {
...
// Compute dynamic route list from SHIFT_ROUTES to keep them harmonized
const uniqueRoutesSet = new Set<string>();
Object.values(SHIFT_ROUTES).forEach(routesObj => {
  Object.keys(routesObj).forEach(routeName => {
    uniqueRoutesSet.add(routeName);
  });
});

export const BUS_ROUTES = Array.from(uniqueRoutesSet).map((routeName, index) => ({
  id: `R${index + 1}`,
  name: routeName
}));
